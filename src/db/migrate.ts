 ...
 * already-applied migrations are skipped (docs/06_DATABASE_DESIGN.md §23,
 * "Applied migration history is immutable").
 *
 * This module never logs a connection string. It is the single place both
 * scripts/db-migrate.mjs (administrative/local use, DATABASE_DIRECT_URL)
 * and the database integration test suite (TEST_DATABASE_URL) apply
 * migrations from, so both paths are provably identical.
 */
