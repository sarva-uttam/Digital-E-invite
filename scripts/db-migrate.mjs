 ...
 * docs/06_DATABASE_DESIGN.md §15/§23 ("Prefer DATABASE_DIRECT_URL for
 * administrative/migration execution"). Safe to run repeatedly:
 * already-applied migrations are skipped by Drizzle's migration-history
 * tracking table.
 *
 ...
 * Next.js/Vite-resolved code (see src/lib/env.ts), so this script avoids
 * importing that module tree and instead calls the Drizzle migrator
 * directly with the small amount of duplicated setup that requires. Tests
 * (vitest, Vite-resolved) reuse src/db/migrate.ts's runMigrations()
 * instead — see src/db/migrations.db.test.ts.
 *
