> **Vision V2 amendment — 2026-08-25 (reconciled):** Routine approval pauses between ordinary dependency-safe tasks, and the earlier three-package commercial structure, are superseded by [`docs/14_OWNER_VISION_V2.md`](./14_OWNER_VISION_V2.md) and `project/DECISIONS.md` (`DEC-025`–`DEC-028`) — see `project/TASKS.md`, the single authoritative execution ledger (`project/TASKS_V2.md` is retired). The wedding-only MVP boundary in this constitution is **not** superseded; it is reaffirmed by `DEC-027`, which also corrects Vision V2's multi-occasion MVP-launch framing. The security, privacy, payment integrity, accessibility, testing, migration safety, and production hard stops in this constitution remain fully binding. Development may otherwise proceed autonomously through dependency-safe work.
